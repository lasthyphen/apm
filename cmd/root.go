...
import (
	...
	"github.com/lasthyphen/apm/apm"
	"github.com/lasthyphen/apm/config"
	"github.com/lasthyphen/apm/constant"
)
...
func New(fs afero.Fs) (*cobra.Command, error) {
	...
	rootCmd.PersistentFlags().String(pluginPathKey, filepath.Join(goPath, "src", "github.com", "lasthyphen", "dijetsnode", "build", "plugins"), "path to dijets plugin directory")
	rootCmd.PersistentFlags().String(credentialsFileKey, "", "path to credentials file")
	rootCmd.PersistentFlags().String(adminAPIEndpointKey, "127.0.0.1:9650/ext/admin", "endpoint for the dijets admin api")
